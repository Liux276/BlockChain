    ...
    >
      ...
      >
        ...
        <el-button
          ...
          type="warning"
          icon="el-icon-delete"
          @click.prevent="removeProposol(index)"
          class="deleteBtn"
        >删除</el-button>
      </el-form-item>
      <el-form-item>
        <el-button
          ...
          type="success"
          icon="el-icon-check"
          @click="submitForm('dynamicInfoForm')"
          class="bottomBtn"
        >提交</el-button>
        <el-button
          v-loading="loading"
        ...
        <el-button
          ...
          type="danger"
          icon="el-icon-delete"
          @click="resetForm('dynamicInfoForm')"
          class="bottomBtn"
        >重置</el-button>
      </el-form-item>
    </el-form>
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          for (let i = 0; i < context.dynamicInfoForm.proposols.length; i++) {
            ...
            ProposalContents[i] =
              context.dynamicInfoForm.proposols[i].proposolContent
          }
          let data = {
          ...
          this.$axios
            .post('/api/contract/newContract', data)
            .then(function(res) {
              context.loading = false
              if (res.status === 200 && res.data.state) {
                context.$router.push('/contractManage/myContractManage')
                context.$notify({
                  type: 'success',
